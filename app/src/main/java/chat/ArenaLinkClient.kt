...
class ArenaLinkClient(private val diagnostics: ChatDiagnostics? = null) {
    ...
    fun connect(host: String, gamePort: Int, name: String, secret: String, useCode: Boolean) {
        ...
        current = session
        log(session, "CONNECT build=U035e protocol=${Alk.PROTOCOL} host=$host tcp_port=${gamePort + 2} name_present=${name.isNotEmpty()} password_present=${secret.isNotEmpty()}")
        thread(name = "arena-link-read", isDaemon = true) {
            try {
                ...
                send(Buf().apply { u16(Alk.PROTOCOL); u8(1); text("ArenaMP U035e", 32); text(session.name, 120) }.frame(Alk.HELLO), session)
